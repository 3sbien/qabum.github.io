...
async function generateQabumPdf(){
  ...
  for(let idx=0;idx<selected.length;idx++){
    ...
    doc.setTextColor(1,33,105);doc.setFont('helvetica','bold');doc.setFontSize(17);doc.text('QSale',12,16);
  ...
  doc.save(`QSale_Catalogo_${today()}.pdf`);toast('PDF generado');
}

async function makeQabumCard(i){
  const c=document.createElement('canvas');c.width=1080;c.height=1350;const x=c.getContext('2d');x.fillStyle=QABUM_WHITE;x.fillRect(0,0,1080,1350);x.fillStyle=QABUM_BLUE;x.fillRect(0,0,1080,118);x.fillStyle=QABUM_RED;x.fillRect(0,118,280,9);x.fillStyle=QABUM_WHITE;x.font='800 36px Arial';x.fillText('QSale',60,72);
...
window.downloadCard=async(id)=>{const i=items.find(x=>x.id===id);if(!i)return;const blob=await makeQabumCard(i);const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${i.title.replace(/[^a-z0-9]+/gi,'_').slice(0,50)}_QSale.png`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);toast('Tarjeta descargada')};
...
document.addEventListener('click',async e=>{
  ...
  if(e.target.closest('#exportBackup')){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();const payload={app:'QSale',version:1,exportedAt:new Date().toISOString(),settings:getSettings(),items:await dbGetAll()};const blob=new Blob([JSON.stringify(payload)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`QSale_Respaldo_${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);localStorage.setItem(BACKUP_KEY,new Date().toISOString());renderStorageNotice();renderSettings();toast('Respaldo descargado')}
},true);
