...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!upstream.ok || !sessionPair) {
      res.status(401).setHeader('Content-Type', 'text/html; charset=utf-8');
      return res.end(page('No se pudo iniciar sesión. Revisa la contraseña.'));
    }

    const value = Buffer.from(sessionPair, 'utf8').toString('base64url');
    const expires = new Date(Date.now() + MAX_AGE * 1000).toUTCString();
    res.setHeader('Set-Cookie', `${APP_COOKIE}=${value}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${MAX_AGE}; Expires=${expires}`);
    res.statusCode = 303;
